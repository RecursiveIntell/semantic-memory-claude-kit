...
| Symptom | Fix |
|---|---|
| `mcp_config.json.example` not parseable | `python3 -m json.tool windsurf/mcp_config.json.example` — should print valid JSON. |
| MCP not loading in Windsurf | Restart Windsurf Cascade after writing the MCP config; check Windsurf's MCP logs. |
| Rule not auto-applying | Verify the rule path with `windsurf/scripts/setup.sh --write-user` produced the expected rule file. |
